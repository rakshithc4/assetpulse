...
/*                                                                            */
/*  AssetPulse note: recolored to the app's own amber/copper identity        */
/*  (design/tokens.json) instead of the original's default fire palette —    */
/*  everything else (physics, shader passes, resource lifecycle) is an       */
/*  unmodified port. Used once, centered behind the login title/logo.        */
/* -------------------------------------------------------------------------- */

export interface BlackHoleProps extends React.HTMLAttributes<HTMLDivElement> {
   ...
   * is dimmed — physically it is g³, and this dials the exponent.
   */
  doppler?: number;
   ...
   * everything else, so near the rim they smear into arcs.
   */
   ...
   * left origin. [0.5, 0.5] centres it.
   */
  focus?: [number, number];
...
void gasAt(vec3 p, float rd, float dt, out float dens, out vec3 tint, out float heat) {
  ...
  // Detail the ray cannot resolve is detail it should not be asking for. So the
  // last octave fades out as the step grows.
  ...
  // Two copies of the disc run the same wind on clocks half a cycle apart, and
  // the picture crossfades from one to the other, so the spiral never winds
  // past one cycle's worth.
  ...
  // properly dark instead of filling in as haze.
...
function hexToLinear(hex: string): [number, number, number] {
  ...
  return srgb.map((v) => (v <= 0.04045 ? v / 12.92 : Math.pow((v + 0.055) / 1.055, 2.4))) as [
    number,
    number,
    number,
  ];
...
export function BlackHole({
  ...
  useEffect(() => {
    ...
    const gl = (canvas.getContext('webgl2', opts) || canvas.getContext('webgl', opts)) as
    ...
    function compile(type: number, src: string): WebGLShader | null {
      ...
      if (!gl!.getShaderParameter(sh, gl!.COMPILE_STATUS)) {
        console.error('black-hole: shader failed —', gl!.getShaderInfoLog(sh) || 'no log (context lost?)');
        gl!.deleteShader(sh);
        return null;
      }
      return sh;
    ...
    if (isGL2) {
      ...
      const ok =
        g2.getExtension('EXT_color_buffer_half_float') || g2.getExtension('EXT_color_buffer_float');
      if (ok) {
    ...
    return () => {
      ...
      // Context deliberately left alive — see shader-background.tsx's identical
      // note: killing it here traps React StrictMode's double-effect cycle.
    };
  }, []);
  ...
  return (
    <div
      ref={hostRef}
      className={`relative isolate h-full w-full overflow-hidden bg-transparent ${className}`}
      {...rest}
    >
